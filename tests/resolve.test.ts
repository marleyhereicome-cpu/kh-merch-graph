...
describe("resolveCandidates", () => {
  ...
  it("KHと無関係なタイトルは候補を返さない", () => {
    const { candidates, weak_matches } = resolveCandidates("ポケモン ぬいぐるみ ピカチュウ", catalog, productLines, 3, [], ipTerms);
  ...
  it("他作品名が含まれる出品はA賞等が一致しても候補を出さない", () => {
    ...
    const { candidates, weak_matches } = resolveCandidates(title, catalog, productLines, 3, otherIpKeywords, ipTerms);
    expect(candidates.length).toBe(0);
    expect(weak_matches.length).toBe(0);
  });
  ...
  it("弾を特定できない場合、同点の弾を行順で一つに絞らず全て返す", () => {
    const { candidates } = resolveCandidates("一番くじ キングダムハーツ D賞", catalog, productLines, 3, [], ipTerms);
