...
describe("resolveCandidates", () => {
  ...
  it("KHと無関係なタイトルは候補を返さない", () => {
    const candidates = resolveCandidates("ポケモン ぬいぐるみ ピカチュウ", catalog, productLines, 3);
  ...
  it("他作品名が含まれる出品はA賞等が一致しても候補を出さない", () => {
    ...
    const candidates = resolveCandidates(title, catalog, productLines, 3, otherIpKeywords);
    expect(candidates.length).toBe(0);
  });
